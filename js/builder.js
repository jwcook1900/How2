...
(function () {
  ...
  */
  ...
  function getFilePayloads(files) {
    ...
    return Promise.all((files || []).map(function (f) {
      return getFilePayload(f).then(function (fp) {
        ...
        // A big PDF can yield both: text for the pages that had a text layer,
        // images for the pages that were scans.
        if (fp.text) texts.push(fp.text);
        if (fp.pages) {
          fp.pages.forEach(function (p) { fileDatas.push(p.data); fileTypes.push(p.type); });
        } else if (!fp.text && fp.data) { fileDatas.push(fp.data); fileTypes.push(fp.type); }
      });
    })).then(function () {
  ...
  // Pulls each page's text layer. Most discharge PDFs are generated by practice
  // software and carry one; a scan doesn't.
  function readPdfText(pdf) {
    var out = [];
    var chain = Promise.resolve();
    for (var n = 1; n <= pdf.numPages; n++) {
      (function (page) {
        chain = chain.then(function () {
          return pdf.getPage(page).then(function (p) {
  ...
  // Turns a too-big PDF into something the AI can read, preferring its text
  // layer: a page sent as text costs a fraction of the same page sent as an
  // image, so a text-layer PDF reads several times faster and more faithfully
  // (no OCR guesswork). Pages without real text are scans, and still go as
  // images. Resolves { text?, pages?: [{data,type}] } or { error }.
  var PDF_PAGE_TEXT_MIN = 180; // below this a page is a scan, not text
  function rasterizePdf(file) {
    ...
    return loadPdfJs().then(function (lib) {
      ...
      return readPdfText(pdf).then(function (pageTexts) {
        var textParts = [], scanned = [];
        pageTexts.forEach(function (p) {
          if (p.text.length >= PDF_PAGE_TEXT_MIN) {
            textParts.push("--- Page " + p.page + " ---\n" + p.text);
          } else {
            scanned.push(p.page);
          }
        });
        var text = textParts.join("\n\n");
        if (!scanned.length) return { text: text }; // fully text — nothing to render
        return renderPdfPages(pdf, scanned, 1600, 0.78).then(function (datas) {
          var total = datas.reduce(function (s, d) { return s + d.length; }, 0);
          // Too heavy at full size (dense scans): one retry, smaller and rougher.
          if (total > PDF_BUDGET) return renderPdfPages(pdf, scanned, 1100, 0.6);
          return datas;
        }).then(function (datas) {
          var total = datas.reduce(function (s, d) { return s + d.length; }, 0);
          if (total > PDF_BUDGET) {
          ...
          return {
            text: text,
            pages: datas.map(function (d) { return { data: d, type: "image/jpeg" }; })
          };
        });
      });
    }).catch(function () {
  ...
  function readFileIntoField(opts) {
    ...
    input.addEventListener("change", function () {
      ...
      getFilePayload(file).then(function (payload) {
        ...
        if (payload.text != null && !payload.pages) { // plain text — drop straight in
        ...
        var aiOpts = { category: ctx.category, question: ctx.question, text: payload.text || null };
        if (payload.pages) {
  ...
  function runVetRefine(btn, note) {
    ...
    btn.disabled = true;
    // The sparkle spins while we wait: a static "Tidying…" on a disabled
    // button gives no sign the second press did anything.
    btn.innerHTML = '<span class="vt-spark">✨</span> Tidying…';
    GotItStore.ai("vetrefine", { text: JSON.stringify(payload), category: "vet discharge" })
      ...
      .catch(function () {
        btn.disabled = false;
        btn.innerHTML = "✨ Tidy up the guide";
